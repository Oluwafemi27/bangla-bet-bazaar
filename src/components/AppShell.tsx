...
import { Home, Gamepad2, Wallet, User, LogIn, Bell } from "lucide-react";
      ...
      <header className="sticky top-0 z-40 backdrop-blur-xl bg-background/80 border-b border-border/50">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 h-14">
          <Link to="/" className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-md bg-gold-gradient flex items-center justify-center font-bold text-gold-foreground">
              বি
            </div>
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <>
                <Link
                  to="/wallet"
                  className="flex items-center gap-2 px-3 h-9 rounded-md bg-card gold-border text-sm font-semibold"
                >
                  <span className="text-gold">{bdt(profile?.balance)}</span>
                </Link>
                <Link
                  to="/notifications"
                  className="h-9 w-9 rounded-md bg-card border border-border flex items-center justify-center"
              ...
              <Link
                to="/auth"
                className="flex items-center gap-1.5 px-4 h-9 rounded-md bg-gold-gradient text-gold-foreground text-sm font-bold glow-gold"
              >
